...
if (!cd_plugin_threshold){
	var cd_plugin_threshold = 5.0;
}
// !DGB! 12/01
...
if (!CD_AUTODOWNLOAD_PLUGIN) {
	var CD_AUTODOWNLOAD_PLUGIN  = "http://intranet.nibr.novartis.intra/departments/SupportFunctions/NITAS/chemdraw";
}
...
function cd_insertObjectStr(tagStr) {
	...
	cd_insertObject(paraArray.type, paraArray.width, paraArray.height, paraArray.name, paraArray.src, paraArray.viewonly, paraArray.shrinktofit, paraArray.dataurl, paraArray.dontcache, paraArray.dockingreferenceid, paraArray.editoutofplace);
}

function getChemdrawObjectString(tagStr) {

    var paraArray = {"type" : "", "width" : "", "height" : "", "name" : "", "src" : "", "viewonly" : "", "shrinktofit" : "", "dataurl" : "", "dontcache" : "", "dockingreferenceid" : "", "editoutofplace" : ""};

    cd_parsePara(tagStr, paraArray);

    var obj= cd_getObject(paraArray.type, paraArray.width, paraArray.height, paraArray.name, paraArray.src, paraArray.viewonly, paraArray.shrinktofit, paraArray.dataurl, paraArray.dontcache, paraArray.dockingreferenceid, paraArray.editoutofplace);
    return 	obj;
}
...
function cd_insertObject(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, dockingreferenceid, editoutofplace) {
	if (cd_currentUsing >= 1 && cd_currentUsing <= 3) {
		//!DGB! 12/01 Add a call to cd_AddToObjectArray
		cd_AddToObjectArray(objName);
	}
	document.write( cd_getSpecificObjectTag(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, dockingreferenceid, editoutofplace) );
}

function cd_getObject(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, dockingreferenceid, editoutofplace) {
    if (cd_currentUsing >= 1 && cd_currentUsing <= 3) {
    //!DGB! 12/01 Add a call to cd_AddToObjectArray
        cd_AddToObjectArray(objName);
    }
    return cd_getSpecificObjectTag(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, dockingreferenceid, editoutofplace);
}
...
function cd_getSpecificObjectTag(mimeType, objWidth, objHeight, objName, srcFile, viewOnly, shrinkToFit, dataURL, dontcache, dockingreferenceid, editoutofplace) {
	...
	if (cd_currentUsing == 1) {	// ActiveX Control
		...
		if (editoutofplace !== null && editoutofplace !== ""){
			buf += " editoutfofplace=\"" + editoutfofplace + "\"";
		}
...
function cd_isCDActiveXInstalled() {
	...
	if (js_canUseTry) {
		...
		document.write("<OBJECT NAME=\"test_150\" WIDTH=0 HEIGHT=0 CLASSID=\"" + CD_CONTROL150CLSID +  "\"><param NAME=ViewOnly VALUE=true></OBJECT>");
		if (document.all("test_150").Selection !== null){
			CD_CONTROL_CLSID = CD_CONTROL150CLSID;
		}
		else {
